...
const moves = {
    ...
    down(player, canvas) {if(!player.finY()) player.posY = maxPos(canvas.height - player.alto, player.posY, player.velocidad)}, 
    left(player, canvas) {if(!player.inicioX()) player.posX = minPos(0, player.posX, player.velocidad)},
    right(player, canvas) {if(!player.finX()) player.posX = maxPos(canvas.width - player.ancho, player.posX, player.velocidad)}, 
}
